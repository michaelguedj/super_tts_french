...
ODT2TXT_MODIF
-----------------
DISCLAIMER: ONE MODIFIES HERE AN ALREADY EXISTING PROGRAM NAMED:
...
if __name__ == "__main__" :
    ...
    # py odt2txt_modif.py exemple.odt
    book = sys.argv[1]
    odt = OpenDocumentTextFile(book)
    unicode = odt.toString()
    lst = unicode.split("###")
    n = len(lst)
    for i in range(1, n): # on passe la premiere partie
        #print(lst[i])  
        #print("#"*50)
        output = f"{book}_{i}.txt"
        f = open(output, "w")
        f.write(lst[i])
        f.close()
